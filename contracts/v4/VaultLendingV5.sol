...
contract VaultLendingV5 {
   ...
   // ====== Loan Struct ======
    ...
    mapping(bytes32 => uint256) private borrowerLoanIndex;
    ...
    function setFeeRate(uint256 platformFeeRate, uint256 lenderFeeRate,uint256 _reserveRateBP ) external onlyAdmin {
        require(platformFeeRate + lenderFeeRate <= BP_DIVISOR, "lender and platform fee > 100%");
        require(_reserveRateBP <= BP_DIVISOR, "fee > 100%");
    ...
    {
        uint256 supply = totalSupply;
        if (supply == 0) return sharesToBurn; // Handle first depositor case

        uint256 nav2 = _nav(); // Use full NAV including poolCash + outstanding principal + reserve
        uint256 amount = (nav2 * sharesToBurn) / supply;
        return amount;
    }
    ...
    function withdraw(uint256 sharesToBurn) external
       whenNotPaused notBlacklisted(msg.sender) onlyWhitelisted(msg.sender) nonReentrant 
      {
       ...
       //uint256 amount = (sharesToBurn * totalAssets) / totalSupply; 
        ...
        uint256 amount = _getAssetForShare(sharesToBurn);
    ...
    function createLoan(bytes32 ref, address merchant, uint256 principal,
     ...
     external onlyCreditOfficer whenNotPaused nonReentrant {
        ...
        require(loans[ref].ref == bytes32(0), "Duplicate loan ref"); 
        ...
        require(fee == platformFee + lenderFee, "fee mismatch");
        ...
        borrowerLoanIndex[ref] = borrowerLoans[borrower].length - 1;
        
        attestationOracle.increaseUsedCredit(borrower, principal);
    ...
    function repayLoan(bytes32 ref, uint256 amount, address borrower) external nonReentrant  {
        ...
        require(amount <= loan.outstanding, "Exceeds outstanding");

        uint256 remaining = amount;
        ITRC20(loan.token).transferFrom(msg.sender, address(this), remaining);
        
         // ---- 2. Fee split ----
        uint256 platformFee = (amount * _platformFeeRate) / BP_DIVISOR;
        uint256 lenderFee = (amount * _lenderFeeRate) / BP_DIVISOR;
        uint256 netAmount = amount - platformFee - lenderFee;


        _platformFee += platformFee;
        _lenderFee += lenderFee;
        totalPrincipalOutstanding -= netAmount;
        poolCash += amount;
         
        loan.outstanding -= amount;
        loan.totalPaid += amount;
        loan.repaidFee += platformFee + lenderFee;
        loan.lastPaymentTs = block.timestamp;
        // ✅ Check accounting
        _checkAccountingInvariant();
    ...
    function _removeLoanFromBorrower(address borrower, bytes32 ref) internal {
        uint256 index = borrowerLoanIndex[ref];
    ...
    function _checkAccountingInvariant() internal view {
        uint256 totalOnChain = ITRC20(token).balanceOf(address(this));
        uint256 totalLiabilities = poolCash + totalPrincipalOutstanding + reserveBalance;
        uint256 totalWithdrawn = totalDisbursedToMerchant + (_platformFee);
        
        require(
            totalLiabilities + totalWithdrawn == totalOnChain,
    ...
    function getMerchantFund(address merchant, address _token) external view onlyAuthorized() returns (uint256) {
        return merchantFund[merchant][_token];
    }
    function getTotalMerchantFund(address _token) external view onlyAuthorized() returns (uint256)
     { 
        return totalMerchantFund[_token];
     }
    ...
    function getShareWorth(uint256 share) external view returns (uint256 amount) {
        if (share == 0 || totalSupply == 0) return 0;
        uint256 navTotal = _nav();
        amount = (navTotal * share) / totalSupply;
    }

    function getVaultBalance(address borrower, address _token) external view returns (uint256) {
        return vault[borrower][_token];
    }
